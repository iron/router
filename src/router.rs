...
impl Router {
    ...
    fn redirect_slash(&self, req : &Request) -> Option<IronError> {
        ...
        if let Some(last_char) = path.chars().last() {
            ...
            {
                let mut path_segments = generic_url.path_segments_mut().expect("We just got this from an Iron URL; it should definitely be a 'can-be-a-base' URL.");
                if last_char == '/' {
            ...
            url = Url::from_generic_url(generic_url).expect("We just got this from an Iron URL; we should be able to put it back into an Iron URL.");
        }
